...
Date: 22/04/2021
Author: Cosmo zhu
Description: A script get mongod-shard metrics
...
class MongoDB(object):
    ...
    def add_metrics(self, k, v):
        ...
        dict_metrics = {}
        dict_metrics['key'] = k + str("[%s]" % self.mongo_port)
    ...
    def get_db_names(self):
        ...
        dict_metrics = {}
        dict_metrics['key'] = 'mongodb.ismaster[' + self.mongo_port + ']'
        if master:
...
if __name__ == '__main__':
    with open("/tmp/mongo_port.log", "r") as port_file:
        ports = port_file.readline()
        for port in ports.split(","):
            if (port is not None) and (port is not ""):
                mongodb = MongoDB(port.strip())
